...
export const useTagsViewStore = defineStore("tagsView", () => {
  ...
  const { storeTabs } = storeToRefs(settingsStore);
  const visitedViews = ref<TagView[]>(useCache().getCacheTabNavList() || []);
  ...
  watch(
    () => storeTabs.value,
    () => {
      handleRecordTabsNav(storeTabs.value);
    }
  );
  watch(
    () => visitedViews.value,
  ...
  const handleRecordTabsNav = (value: boolean) => {
    if (value) useCache().setCacheTabNavList(visitedViews.value);
